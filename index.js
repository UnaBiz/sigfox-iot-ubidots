...
function wrap(scloud) {
    ...
    function transformBody(req, body0) {
        ...
        if (body.lat) {
            body.baseStationLat = body.lat;
            delete body.lat;
        }
        if (body.lng) {
            body.baseStationLng = body.lng;
            delete body.lng;
        }
        if (!latFields || !lngFields)
            return body;
        ...
        for (let i = 0; i < len; i += 1) {
            ...
            if (latField.length === 0 || lngField.length === 0)
                continue;
            if (!body[latField] || !body[lngField])
                continue;
    ...
    function containsLocation(req, body) {
        ...
        if (body.deviceLat && body.deviceLng)
            return true;
        return false;
    }
    function copyLocationSensorFields(req, device, body) {
        ...
        return (scloud.getDeviceState(req, device)
            ...
            if (containsLocation(req, body)) {
                ...
                locationFields.forEach((key) => {
                    if (state[key] && key !== 'metadata')
                        newBody[key] = state[key];
                });
    ...
    function task(req, device, body0, msg) {
        ...
        wrapCount += 1;
        console.log({ wrapCount, body0 }); //  Count how many times the wrapper was reused.
...
exports.main = process.env.FUNCTION_NAME ? require('sigfox-gcloud/main').getMainFunction(wrapper, wrap, package_json) //  Google Cloud.
    : (event, context, callback) => {
        ...
        if (require('fs').existsSync('/tmp/autoinstall.js'))
            return afterExec(null); //  Already downloaded.
